...
import {
  createAccountSchema,
  CreateUserRequest,
} from "@/models/requests/user/create-user-request";
import {
  parseDate,
  today,
  getLocalTimeZone,
...
const CreateAccountForm: React.FC<CreateAccountFormProps> = ({ onClose }) => {
  ...
  const { getByUsername, isAvailable, setIsAvailable, isGettingByUsername } =
    useGetByUsername();
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center gap-3 mb-2">
            {watch("imageFile") ? (
              <FileAvatar
                file={watch("imageFile") as FileType}
                onRemove={clearPicture}
                onAdd={handleEditPictureClicked}
              />
            ) : (
              <Avatar
                className="transition-all"
                name={
                  isAvailable || watch("username") !== ""
                    ? watch("username")
                    : "adam"
                }
                size={70}
            ...
            <Button
              ...
              onPress={
                watch("imageFile") ? clearPicture : handleEditPictureClicked
              }
          ...
          {isAvailable && (
            <>
              ...
              <div className="flex w-full items-center gap-2 opacity-40">
                <label className="text-xs opacity-50  font-semibold text-nowrap">
                  Security
                </label>
              ...
              <DateInput
                ...
                labelPlacement="outside"
                onChange={(e) =>
                  setValue("dayOfBirth", e ?? today(getLocalTimeZone()))
                }
                value={parseDate(
                  watch("dayOfBirth")?.toString() ??
...
export const FileAvatar: React.FC<FileAvatarProps> = ({
  file,
  onRemove,
  onAdd,
}) => (
