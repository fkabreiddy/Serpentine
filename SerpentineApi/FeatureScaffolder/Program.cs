...
internal class Program
{
    static async Task Main(string[] args)
    {
        ...
        var featureName = AnsiConsole.Ask<string>("Como se llama la [green]feature?[/] por ejemplo, CreateChannel: ");
        ...
        var featureClassName = AnsiConsole.Prompt<string>(
            ...
            .AddChoices<string>(new string[] { "Channel", "Group", "User", "Message", "ChannelMember", "GroupAccess" })
        );


        var responseClassName = AnsiConsole.Ask<string>("Que tipo de dato va a retornar el endpoint? Puede ser una clase o struct?");
        ...
        var confirmation = AnsiConsole.Prompt(
            new SelectionPrompt<string>()
                .Title($"Seguro que quieres crear la feature ({folderType}) {method} [green]{featureClassName}[/] :: [blue]{route} => {responseClassName}[/]: ?")
        ...
        await AnsiConsole.Status()
            .StartAsync("[green]Generando {featureClassName} en {folderType}...[/]", async ctx =>
            {
                var featureFolder = Path.Combine("../SerpentineApi/Features", featureClassName + "Features", folderType);
