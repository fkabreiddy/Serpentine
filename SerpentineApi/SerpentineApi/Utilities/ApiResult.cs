...
public class NotFoundApiResult : Failure
{
    public NotFoundApiResult(
        string message = "The requested resource does not exist or has been deleted.",
...
{
    public ForbiddenApiResult(
        string message = "The requested resource is forbidden.",
        List<string>? messages = null
    )
    {
...
public class ValidationApiResult : Failure
{
    public ValidationApiResult(
        string message = "One or more parameters are invalid or missing or are invalid. Try again.",
        List<string>? messages = null
    )
    {
...
public class BadRequestApiResult : Failure
{
    public BadRequestApiResult(
        string message = "One or more parameters are invalid or missing.",
        List<string>? messages = null
    )
    {
        Build(message, 404, messages);
...
{
    public ConflictApiResult(
        string message = "The resource already exists or cannot be modified in its current state.",
        List<string>? messages = null
    )
    {
