...
public static class ChannelEntityExtensions
{
   ...
   public static async Task<List<Channel>> GetChannelsWithJustMyMembershipByNameOrId(this DbSet<Channel> channelsContext, Ulid? channelId, string? channelName, Ulid userId, CancellationToken token = default)
   {

      IQueryable<Channel> query = channelsContext
         .Include(ch => ch.Members.Where(m => m.UserId == userId))
         .AsNoTracking()
         .AsSplitQuery().Where(ch => ch.Name != null);
