...
namespace SerpentineApi.Features.MessageFeatures.Actions;
            ...
            {
                {
                    ...
                    return ResultsBuilder.Match<List<MessageResponse>>(new SuccessApiResult<List<MessageResponse>>(t0));
        ...
        .Experimental()
        .WithOpenApi()
        .WithDescription("Fetches a list of messages with a certain group id. Requires Authorization. \n Requires CORS.")
        ...
        internal class GetMessagesByGroupIdEndpointHandler(

            SerpentineDbContext context
        ) : IEndpointHandler<GetMessagesByGroupIdRequest, OneOf<List<MessageResponse>, Failure>>
        {
            public async Task<OneOf<List<MessageResponse>, Failure>> HandleAsync(GetMessagesByGroupIdRequest request, CancellationToken cancellationToken = default)
    {
        var group = await context.Groups.AsNoTracking().FirstOrDefaultAsync(g => g.Id == request.GroupId);
        ...
        if (await context.ChannelMembers.AsNoTracking().FirstOrDefaultAsync(cm => cm.ChannelId == group.ChannelId && cm.UserId == request.CurrentUserId, cancellationToken) is var membership && membership is null)
        {
