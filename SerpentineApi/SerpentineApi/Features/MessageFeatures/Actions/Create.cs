...
public class CreateMessageRequest
    : RequestWithUserCredentials,
        IRequest<OneOf<MessageResponse, Failure>>
{
    [
        JsonPropertyName("groupId"),
        FromForm,
        Required,
        Description("The id of the group that this message belongs to")
    ]
    ...
    [
        ...
        FromForm(Name = "isNotification"),
        Description(
            "Set if the message is a notification with no sender. By default is set to false"
        )
    ]
        ...
        app.MapPost(
                ...
                async (
                    ...
                    await executor.ExecuteAsync<MessageResponse>(async () =>
                    {
                        ...
                        return ResultsBuilder.Match<MessageResponse>(
                            new SuccessApiResult<MessageResponse>(t0)
                        );
                    })
            )
            .DisableAntiforgery()
            ...
            .WithOpenApi()
            .WithDescription(
                "Creates a message ina  group and send it through signalr. Requires Authorization. \n Requires CORS."
            )
...
{
    public async Task<OneOf<MessageResponse, Failure>> HandleAsync(
    ...
    )
    {
        if (
            await context
                .Groups.AsNoTracking()
                .FirstOrDefaultAsync(g => g.Id == messageRequest.GroupId, cancellationToken)
                is var group
            && group is null
        )
        {
        ...
        if (
            await context
                ...
                .FirstOrDefaultAsync(
                    cm =>
                        cm.ChannelId == group.ChannelId
                        && cm.UserId == messageRequest.CurrentUserId,
                    cancellationToken
                )
                is var membership
            && membership is null
        ...
        {
            await channelsHub
                .Clients.Group(group.ChannelId.ToString() ?? string.Empty)
                .SendMessage(new HubResult<MessageResponse>(result.ToResponse()));
        }
        else
        {
            var adminsAndOwer = await context
                .ChannelMembers.AsNoTracking()
                .IgnoreAutoIncludes()
                .Where(cm => cm.IsOwner || cm.IsAdmin)
                .Select(cm => new ChannelMember() { UserId = cm.UserId })
                .ToListAsync();

            await channelsHub
                .Clients.Users(adminsAndOwer.Select(x => x.UserId.ToString()))
                .SendMessage(new HubResult<MessageResponse>(result.ToResponse()));
        }
