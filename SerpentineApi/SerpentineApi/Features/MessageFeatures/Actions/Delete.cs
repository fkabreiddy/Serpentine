...
namespace SerpentineApi.Features.MessageFeatures.Actions;
        ...
        .Experimental()
        .WithOpenApi()
        .WithDescription("Deletes a channel with a certain id.Requires Authorization. \n Requires CORS.")
        ...
        {
    public async Task<OneOf<string, Failure>> HandleAsync(DeleteMessageRequest request, CancellationToken cancellationToken = default)
    {

            bool hasPermission = false;
            
            var channelId = await context.Messages
            .AsNoTracking()
            .IgnoreAutoIncludes()
            ...
            if (!await context.Messages.AnyAsync(m => m.Id == request.MessageId && m.Sender != null && m.SenderId == request.CurrentUserId, cancellationToken))
        {
            ...
            if (!(UlidHelper.IsValid(channelId) && !await context.ChannelMembers.AnyAsync(c => c.ChannelId == channelId && (c.IsOwner || c.IsAdmin), cancellationToken)))
            {
        ...
        else
        {
            hasPermission = true;
                ...
                if (!hasPermission)
                {
                    ...
                    var deletion = await context.Messages.Where(m => m.Id == request.MessageId).ExecuteDeleteAsync(cancellationToken);

                if (deletion <= 0)
                {
                    return new NotFoundApiResult();
                }
                await channelsHub.Clients.Group(channelId.ToString()).SendMessageDeleted(new HubResult<string>(request.MessageId.ToString()));
