...
public class UpdateGroupRequest : RequestWithUserCredentials, IRequest<OneOf<GroupResponse, Failure>>
{

    [FromBody, Required, JsonPropertyName("groupId"), Description("The id of the group to update")]
    public Ulid GroupId { get; set; }

    [FromBody, Required, MaxLength(100), MinLength(3), RegularExpression(@"^[a-zA-Z0-9_]+$"), JsonPropertyName("name"), Description("The name of the group")]
    public required string Name { get; set; }

    [FromBody, Required, JsonPropertyName("public"), Description("Whether a group is public or not")]
    public bool Public { get; set; }

    [FromBody, Required, JsonPropertyName("requiresOverage"), Description("Whether a group requires the member of the channel to be overage or not")]
...
public class UpdateGroupRequestValidator : AbstractValidator<UpdateGroupRequest>
{
    public UpdateGroupRequestValidator()
    {
        ...
        RuleFor(x => x.RequiresOverage).NotNull().WithMessage("Especify if the group requires overage or not");
    ...
    internal class UpdateGroupEndpoint : IEndpoint
    {
        ...
        public void MapEndpoint(IEndpointRouteBuilder app)
        {
            ...
            .Experimental()
            .WithOpenApi()
            .WithDescription("Updates a group with a certain id. Requires Authorization. \n Requires CORS.")
    ...
    internal class UpdateGroupEndpointHandler(SerpentineDbContext context) : IEndpointHandler<UpdateGroupRequest, OneOf<GroupResponse, Failure>>
    {
        public async Task<OneOf<GroupResponse, Failure>> HandleAsync(UpdateGroupRequest request, CancellationToken cancellationToken = default)
        {

            request.Name.Trim();

            if (await context.Groups.AsTracking().AsSplitQuery().FirstOrDefaultAsync(g => g.Id == request.GroupId, cancellationToken) is var group && group is null)
            {
                return new NotFoundApiResult("Group not found");
            }

            
           if (await context.Groups.AnyAsync(x =>
                    x.ChannelId == group.ChannelId && x.Name.ToLower() == request.Name.ToLower() && x.Id != group.Id
                )
            )
            ...
            if (await context.ChannelMembers.AsNoTracking().AsSplitQuery().FirstOrDefaultAsync(cm => cm.ChannelId == group.ChannelId && cm.UserId == request.CurrentUserId, cancellationToken) is var membership && membership is null)
            {
            ...
            if (await context.Groups.GetGroupWithJustMyAccessByGroupId(request.GroupId, request.CurrentUserId) is var result && result is null)
            {
                return new NotFoundApiResult("Group not found");
            }
