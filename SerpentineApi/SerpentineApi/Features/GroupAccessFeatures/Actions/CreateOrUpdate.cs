...
public class CreateGroupAccessRequest
    : RequestWithUserCredentials,
        IRequest<OneOf<GroupAccessResponse, Failure>>
{
    [
        Required,
        JsonPropertyName("groupId"),
        FromBody,
        Description("The id of the last acceceed group")
    ]
    public Ulid GroupId { get; set; }

    [
        JsonPropertyName("lastReadMessageDate"),
        FromBody,
        Description("The date of the last message the user red")
    ]
...
internal class CreateGroupAccessEndpointHandler(SerpentineDbContext context)
    : IEndpointHandler<CreateGroupAccessRequest, OneOf<GroupAccessResponse, Failure>>
{
    public async Task<OneOf<GroupAccessResponse, Failure>> HandleAsync(
        CreateGroupAccessRequest request,
        CancellationToken cancellationToken = default
    )
    {
        var group = await context
            .Groups.IgnoreAutoIncludes()
            .AsNoTracking()
        ...
        if (
            await context
                .ChannelMembers.IgnoreAutoIncludes()
                .AsNoTracking()
                .FirstOrDefaultAsync(x =>
                    x.ChannelId == group.ChannelId && x.UserId == request.CurrentUserId
                )
                is var membership
            && membership is null
        )
        ...
        if (
            (!isAdminOrOwner && !group.Public)
            || (group.RequiresOverage && (request.CurrentUserAge < 18))
        )
        {
            return new ForbiddenApiResult(
                "You don't have permission to register your access in this group"
            );
        }

        var existingAccess = await context
            .GroupAccesses.AsNoTracking()
            .FirstOrDefaultAsync(
                ga => ga.UserId == request.CurrentUserId && ga.GroupId == request.GroupId,
                cancellationToken
            );
        GroupAccess? result;
        ...
        {
            await context
                .GroupAccesses.Where(ga => ga.Id == existingAccess.Id)
                .ExecuteUpdateAsync(
                    x => x.SetProperty(y => y.LastReadMessageDate, request.LastReadMessageDate),
                    cancellationToken
                );
            ...
            result = await context
                .GroupAccesses.AsNoTracking()
                .FirstOrDefaultAsync(ga => ga.Id == existingAccess.Id, cancellationToken);
        }
        else
        {
            ...
            result = await context
                .GroupAccesses.AsNoTracking()
                .FirstOrDefaultAsync(ga => ga.Id == creation.Id, cancellationToken);
        }
