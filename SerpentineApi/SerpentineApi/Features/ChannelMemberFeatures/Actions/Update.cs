...
using SerpentineApi.Helpers;
        ...
        public class UpdateChannelMemberRequest :RequestWithUserCredentials, IRequest<OneOf<ChannelMemberResponse, Failure>>
        {
            [JsonPropertyName("channelMemberId"), Required, FromBody, Description("The id of the channelId to update")]
            public Ulid ChannelMemberId { get; set; }


            [JsonPropertyName("shouldBeAdmin"), FromBody,
             Description("Stablish wether Admin role should be updated. If null, no changes will be applied.")]
        ...
        internal class UpdateChannelMemberEndpointHandler(
            SerpentineDbContext context
            ) : IEndpointHandler<UpdateChannelMemberRequest, OneOf<ChannelMemberResponse, Failure>>
        {
            public async Task<OneOf<ChannelMemberResponse, Failure>> HandleAsync(UpdateChannelMemberRequest request, CancellationToken cancellationToken = default)
            {
                var channelMember = await context.ChannelMembers.FirstOrDefaultAsync(cm => cm.Id == request.ChannelMemberId, cancellationToken);

                if (channelMember is null)
                    return new NotFoundApiResult("Membership not found");

                if (!await context.ChannelMembers.AnyAsync(
                        cm => cm.ChannelId == channelMember.ChannelId && cm.UserId == request.CurrentUserId &&
                              cm.IsOwner, cancellationToken))
                {
                ...
                var result = await context.ChannelMembers.AsNoTracking().FirstOrDefaultAsync(cm => cm.Id == channelMember.Id, cancellationToken);
