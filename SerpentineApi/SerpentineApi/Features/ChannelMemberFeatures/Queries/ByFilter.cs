...
public class GetChannelMembersByFilterRequest
...
public class GetChannelMembersByFilterRequestValidator
    : AbstractValidator<GetChannelMembersByFilterRequest>
{
...
internal class GetChannelMemberByFilterEndpoint : IEndpoint
{
    ...
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapGet(
            ...
            .WithDescription(
                "Returns a channel member with a certain ChannelId and UserId. Requires Authorization. Requires CORS."
            );
    }
}

internal class GetChannelMembersByFilterEndpointHandler(SerpentineDbContext context)
    : IEndpointHandler<GetChannelMembersByFilterRequest, OneOf<ChannelMemberResponse, Failure>>
{
    public async Task<OneOf<ChannelMemberResponse, Failure>> HandleAsync(
        GetChannelMembersByFilterRequest request,
    ...
    )
    {
        var channel = await context
            ...
            .AsSplitQuery()
            .FirstOrDefaultAsync(
                ch => ch.ChannelId == request.ChannelId && ch.UserId == request.CurrentUserId,
                cancellationToken
            );
