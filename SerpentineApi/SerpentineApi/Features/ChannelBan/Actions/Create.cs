...
public class CreateChannelBanRequest
    : RequestWithUserCredentials,
        IRequest<OneOf<ChannelBanResponse, Failure>>
{
...
public class CreateChannelBanRequestValidator : AbstractValidator<CreateChannelBanRequest>
{
    public CreateChannelBanRequestValidator()
    {
        ...
        RuleFor(x => x.Reason)
            .NotNull()
            .WithMessage("There must be valid reason to ban this user")
            .MaximumLength(300)
            .WithMessage("The reason for baning this user must not exceed 300 characters")
            .MinimumLength(5)
            .WithMessage("There reason must be larger than 5 characters");
    }
}
                    ...
                    {
                        {
                            ...
                            var t0 = result.AsT0;
                            await activeUsersHub
                                .Clients.User(t0.UserId.ToString())
                                .SendUserBanned(new HubResult<ChannelBanResponse>(t0));
                            return ResultsBuilder.Match<ChannelBanResponse>(
                                new SuccessApiResult<ChannelBanResponse>(t0)
                            );
                        }
                    })
            )
...
internal class CreateChannelBanEndpointHandler(SerpentineDbContext context)
    : IEndpointHandler<CreateChannelBanRequest, OneOf<ChannelBanResponse, Failure>>
{
    public async Task<OneOf<ChannelBanResponse, Failure>> HandleAsync(
        CreateChannelBanRequest request,
        CancellationToken cancellationToken = default
    )
    {
        if (
            await context.ChannelBans.AnyAsync(
                x => x.ChannelId == request.ChannelId && x.UserId == request.UserId,
                cancellationToken
            )
        )
        ...
        if (membership.IsAdmin || membership.IsOwner)
        {
            return new ForbiddenApiResult(
                "You cant ban an administrator or the owner of the channel"
            );
        }
        ...
        try
        {
            ...
            var deletedRows = await context
                .ChannelMembers.Where(cm =>
                    cm.UserId == request.UserId && cm.ChannelId == request.ChannelId
                )
