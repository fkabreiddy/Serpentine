using System.Text.Json.Serialization;
        ...
        {
            [FromForm(Name = "bannerPictureFile"), JsonPropertyName("bannerPictureFile"),  FileExtensions(Extensions = "jpg, png, webp, img, jpeg")]
        ...
        .Experimental()
        .WithOpenApi()
        .WithDescription("Updates or delete the banner of a channel with a certain Id.Requires Authorization. \n Requires CORS.")
        ...
        internal class UpdateBannerEndpointHandler(CloudinaryService cloudinary, SerpentineDbContext context) : IEndpointHandler<UpdateBannerRequest, OneOf<string, Failure>>
        {
            public async Task<OneOf<string, Failure>> HandleAsync(UpdateBannerRequest request, CancellationToken cancellationToken = default)
            {
                if (!await context.ChannelMembers.AnyAsync(
                        cm => (cm.IsAdmin || cm.IsOwner) && cm.ChannelId == request.ChannelId, cancellationToken))
                {
                    return new ForbiddenApiResult("You don't have permisson to change the banner of this channel");
                ...
                {
                    var deletionResult = await cloudinary.DeleteImageAsync(request.ChannelId.ToString(), CloudinaryFolders.ChannelBanners.ToString());

                    if (!deletionResult.TaskSucceded())
                    {
                        return new NotFoundApiResult($"We had some errors deleting the banner: {deletionResult.Message}", deletionResult.Errors);
                    }
                    
                    await context.Channels
                        .Where(ch => ch.Id == request.ChannelId)
                        .ExecuteUpdateAsync(x => 
                            x.SetProperty(y => y.BannerPicture, ""),
                            cancellationToken
                        );
                ...
                else
                {
                    ...
                    var insertionResult = await cloudinary.UploadImage(request.BannerPictureFile, CloudinaryFolders.ChannelBanners.ToString(), request.ChannelId.ToString());

                    if (!insertionResult.TaskSucceded())
                    {
                        return new BadRequestApiResult($"We had some errors adding the banner: {deletionResult.Message}", deletionResult.Errors);
                    ...
                    await context.Channels
                        .Where(ch => ch.Id == request.ChannelId)
                        .ExecuteUpdateAsync(x => 
                                x.SetProperty(y => y.BannerPicture, insertionResult.Data),
                            cancellationToken
                        );
