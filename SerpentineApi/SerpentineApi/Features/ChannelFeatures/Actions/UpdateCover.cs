using System.Text.Json.Serialization;
        ...
        {
            [FromForm(Name = "coverPictureFile"), JsonPropertyName("coverPictureFile"),  FileExtensions(Extensions = "jpg, png, webp, img, jpeg")]
        ...
        .Experimental()
        .WithOpenApi()
        .WithDescription("Updates or deletes the cover of a channel with a certain id. Requires Authorization. \n Requires CORS.")
        ...
        internal class UpdateCoverEndpointHandler(
            CloudinaryService cloudinary,
            SerpentineDbContext context
        ) : IEndpointHandler<UpdateCoverRequest, OneOf<string, Failure>>
        {
            public async Task<OneOf<string, Failure>> HandleAsync(UpdateCoverRequest request, CancellationToken cancellationToken = default)
            {
                ...
                {
                    var deletionResult = await cloudinary.DeleteImageAsync(request.ChannelId.ToString(), CloudinaryFolders.ChannelCovers.ToString());
                    ...
                    await context.Channels
                        ...
                        .ExecuteUpdateAsync(x => 
                            x.SetProperty(y => y.CoverPicture, ""),
                            cancellationToken
                        );
                ...
                else
                {
                    ...
                    var insertionResult = await cloudinary.UploadImage(request.CoverPictureFile, CloudinaryFolders.ChannelCovers.ToString(), request.ChannelId.ToString());
                    ...
                    await context.Channels
                        .Where(ch => ch.Id == request.ChannelId)
                        .ExecuteUpdateAsync(x => 
                                x.SetProperty(y => y.CoverPicture, insertionResult.Data),
                            cancellationToken
                        );
