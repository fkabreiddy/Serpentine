...
{
    public class GetManyByIdOrNameRequest : RequestWithUserCredentials, IRequest<OneOf<List<ChannelResponse>, Failure>>
    {
        [JsonPropertyName("channelId"), FromQuery(Name = "channelIds"), MinLength(1)]
        public Ulid? ChannelId { get; set; }

        [JsonPropertyName("channelName"), FromQuery(Name = "channelName"), MinLength(0), MaxLength(100)]
    ...
    public class GetManyByIdOrNameRequestValidator : AbstractValidator<GetManyByIdOrNameRequest>
    {
        public GetManyByIdOrNameRequestValidator()
        {
            RuleFor(x => x.ChannelId)
                .Must(x => x is not null ? UlidHelper.IsValid(x ?? new()) : true).WithMessage("The channel Id must be a valid ULID or null");

            RuleFor(x => x.ChannelName)
                .MinimumLength(0).WithMessage("The channel name must be at least 1 character long or null")
                .MaximumLength(100).WithMessage("The channel name cannot exceed 100 characters or null")
        ...
        public void MapEndpoint(IEndpointRouteBuilder app)
        {
            app.MapGet(
                ...
                ) =>
                {
                    ...
                    {
                        request.SetUserId(UserIdentityRequesterHelper.GetUserIdFromClaims(context.User));
                        ...
                        return ResultsBuilder.Match<List<ChannelResponse>>(new SuccessApiResult<List<ChannelResponse>>(channels));
                    });
                }).DisableAntiforgery()
                    ...
                    .WithDescription($"Return a list of {nameof(ChannelResponse)} by their Id or Name. Requires {nameof(GetManyByIdOrNameRequest)}. Returns a list of {nameof(ChannelResponse)}")
                    .WithName(nameof(GetManyByIdOrNameEndpoint));
        }
    }
    
    internal class GetManyByIdOrNameEndpointHandler(SerpentineDbContext dbContext) : IEndpointHandler<GetManyByIdOrNameRequest, OneOf<List<ChannelResponse>, Failure>>
    {
      

        public async Task<OneOf<List<ChannelResponse>, Failure>> HandleAsync(GetManyByIdOrNameRequest request, CancellationToken cancellationToken)
        {
            var channels = await dbContext.Channels.GetChannelsWithJustMyMembershipByNameOrId(request.ChannelId, request.ChannelName, request.CurrentUserId, cancellationToken);
