...
public class GetChannelByIdRequest :RequestWithUserCredentials, IRequest<OneOf<ChannelResponse, Failure>>
{
...
public class GetChannelByIdEndpoint : IEndpoint
{
    ...
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapGet(
            ...
            ) =>
            {
                return await executor.ExecuteAsync<ChannelResponse>(async () =>
                {
                    ...
                    return ResultsBuilder.Match<ChannelResponse>(new SuccessApiResult<ChannelResponse>(t0));
        ...
        .WithDescription($"Returns a {nameof(ChannelResponse)} matching the giving Id. Accepts: {nameof(GetChannelByIdRequest)}. Returns a {nameof(ChannelResponse)} or a {nameof(Failure)}");
        
    }
}

internal class GetChannelByIdEndpointHandler(
...

{
    ...
    public async Task<OneOf<ChannelResponse, Failure>> HandleAsync(GetChannelByIdRequest request, CancellationToken cancellationToken)
    {
        Channel? channel =
            await context.Channels
                .GetChannelsWithJustMyMembershipByChannelId(request.ChannelId, request.ChannelId,
                cancellationToken);
